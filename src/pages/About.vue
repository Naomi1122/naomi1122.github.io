
<script setup lang="ts">
import { useI18n } from 'vue-i18n';

import {  ref } from 'vue';
import { useTheme } from '/@/composables';



const { t, availableLocales, locale } = useI18n();

const toggleLocales = () => {
  const locales = availableLocales;
  locale.value =
    locales[(locales.indexOf(locale.value) + 1) % locales.length];
};

const { toggleDark } = useTheme();

const show = ref(false);

setTimeout(() => {
  show.value = true;
}, 1000);

</script>
<template>

  <div class="container max-w-3xl mx-auto mt-10">
  <div class="container-panel">

    <h1>About Us</h1>
    <p class="center-text">
        When the dog wears this clothes, the owner can check what the dog is doing on the mobile phone and computer anytime, anywhere. 
        The data will be transmitted to Firebase for storage, so that the owner can look back at any time.
      </p>

    <footer class="text-center">
      <ul class="flex justify-between w-1/3 mx-auto mb-8">
        <li class="cursor-pointer text-2xl">
          <a
            href="#"
            @click="toggleLocales"
            class="footer-link text-cyan-700 hover:text-cyan-500"
            :title="t('toggle_language')"
          >
            <i class="i-ph-translate-bold" />
          </a>
        </li>
        <li class="cursor-pointer text-2xl">
          <a
            href="#"
            @click="toggleDark()"
            class="text-cyan-700 hover:text-cyan-500"
            :title="t('toggle_theme')"
          >
            <i i="ph-sun dark:ph-moon" />
          </a>
        </li>
        <li class="cursor-pointer text-2xl">
          <a
            href="https://github.com/Naomi1122/doggydynamic/tree/main"
            rel="noreferrer"
            target="_blank"
            class="footer-link text-cyan-700 hover:text-cyan-500"
            title="Github repo"
          >
            <i class="i-ph-github-logo" />
          </a>
        </li>
      </ul>

      <span class="text-xs"
        >{{ t('made_by') }}
        <a
          class="footer-link text-cyan-400 hover:text-cyan-500"
          href="https://github.com/Naomi1122"
          rel="noreferrer"
          target="_blank"
          >Naomi Cheng</a
        ></span
      >
    </footer>
    </div>
  </div>

</template>

<style>
/* a,
.footer-link {
  @apply transition-all ease-out duration-100;
}

.footer-link {
  opacity: 0.8;
} */
.container {
  width: 100%;
  height: 100%; /* Adjust the height to fit your needs */
  position: absolute;
  z-index:100;
  top:50%;
  left:50%;
  transform:translate(-50%,-50%);
  overflow:hidden
}

.container-panel {
  width: 60%;
  height: 30%; /* Adjust the height to fit your needs */
  text-align:center;
  margin:0 auto;
}


.about {
    text-align: center;
    padding: 20px;
  }
  
  h1 {
    font-size: 30px;
    font-family: 'Fira sans', sans-serif;
    margin-bottom: 10px;
  }
  
  .center-text {
    font-size: 18px;
  }

</style>


  