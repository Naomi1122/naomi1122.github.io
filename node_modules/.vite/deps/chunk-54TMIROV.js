// node_modules/.pnpm/@vueuse+shared@9.7.0_vue@3.2.45/node_modules/@vueuse/shared/index.mjs
import { shallowRef, watchEffect, readonly, unref, ref, isVue3, watch, customRef, getCurrentScope, onScopeDispose, effectScope, provide, inject, isRef, computed, reactive, toRefs as toRefs$1, toRef, isVue2, set as set$1, getCurrentInstance, onBeforeMount, nextTick, onBeforeUnmount, onMounted, onUnmounted, isReactive } from "vue-demi";
...
// node_modules/.pnpm/@vueuse+core@9.7.0_vue@3.2.45/node_modules/@vueuse/core/index.mjs
import { isRef as isRef2, ref as ref2, watchEffect as watchEffect2, computed as computed2, inject as inject2, unref as unref2, watch as watch2, getCurrentInstance as getCurrentInstance2, customRef as customRef2, onUpdated, reactive as reactive2, shallowRef as shallowRef2, onMounted as onMounted2, markRaw, getCurrentScope as getCurrentScope2, readonly as readonly2, nextTick as nextTick2, isVue2 as isVue22, set as set2, del, onBeforeUpdate } from "vue-demi";
function computedAsync(evaluationCallback, initialState, optionsOrRef) {
  ...
  const current = ref2(initialState);
  let counter = 0;
...
var createUnrefFn = (fn) => {
  return function(...args) {
    return fn.apply(this, args.map((i) => unref2(i)));
  };
};
...
function useEventListener(...args) {
  ...
  const stopWatch = watch2(() => unrefElement(target), (el) => {
    cleanup();
    if (!el)
      return;
...
function useBase64(target, options) {
  ...
  if (isRef2(target) || isFunction(target))
...
function useCloned(source, options = {}) {
  ...
  function sync() {
    cloned.value = clone(unref2(source));
  }
  if (!manual && isRef2(source)) {
    watch2(source, sync, __spreadProps$8(__spreadValues$k({}, options), {
      deep,
      immediate
    }));
  } else {
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  const data = (shallow ? shallowRef2 : ref2)(defaults2);
  if (!storage) {
    try {
...
function useColorMode(options = {}) {
  ...
  const store = storageRef || (storageKey == null ? ref2(initialValue) : useStorage(storageKey, initialValue, storage, { window: window2, listenToStorageChanges }));
  ...
  if (emitAuto)
    watch2(preferredMode, () => onChanged(state.value), { flush: "post" });
...
function useCssVar(prop, target, { window: window2 = defaultWindow, initialValue = "" } = {}) {
  ...
  watch2([elRef, () => resolveUnref(prop)], ([el, prop2]) => {
    var _a2;
    if (el && window2) {
...
function useCycleList(list, options) {
  var _a2;
  const state = shallowRef2((_a2 = options == null ? void 0 : options.initialValue) != null ? _a2 : list[0]);
...
function useDisplayMedia(options = {}) {
  var _a2;
  const enabled = ref2((_a2 = options.enabled) != null ? _a2 : false);
...
function useDraggable(target, options = {}) {
  ...
  const position = ref2((_c = resolveUnref(options.initialValue)) != null ? _c : { x: 0, y: 0 });
  ...
  return __spreadProps$42(__spreadValues$e({}, toRefs(position)), {
    position,
...
function useElementSize(target, initialSize = { width: 0, height: 0 }, options = {}) {
  ...
  watch2(() => unrefElement(target), (ele) => {
...
function useElementVisibility(element, { window: window2 = defaultWindow, scrollTarget } = {}) {
  ...
  watch2(() => unrefElement(element), () => testBounding(), { immediate: true, flush: "post" });
  if (window2) {
...
function useFetch(url, ...args) {
  ...
  const error = shallowRef2(null);
  const data = shallowRef2(initialData);
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported.value)
      return;
    const [handle] = await window2.showOpenFilePicker(__spreadValues$a(__spreadValues$a({}, unref2(options)), _options));
  ...
  async function create(_options = {}) {
    if (!isSupported.value)
      return;
    fileHandle.value = await window2.showSaveFilePicker(__spreadValues$a(__spreadValues$a({}, unref2(options)), _options));
    data.value = void 0;
    await updateFile();
    await updateData();
  }
  ...
  async function updateData() {
    var _a2, _b;
    if (unref2(dataType) === "Text")
      data.value = await ((_a2 = file.value) == null ? void 0 : _a2.text());
    if (unref2(dataType) === "ArrayBuffer")
      data.value = await ((_b = file.value) == null ? void 0 : _b.arrayBuffer());
    if (unref2(dataType) === "Blob")
      data.value = file.value;
  }
  watch2(() => unref2(dataType), updateData);
  return {
    isSupported,
    data,
    file,
...
var useImage = (options, asyncStateOptions = {}) => {
  ...
  watch2(() => resolveUnref(options), () => state.execute(asyncStateOptions.delay), { deep: true });
  return state;
};
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive2(useScroll(element, __spreadProps$22(__spreadValues$82({}, options), {
    offset: __spreadValues$82({
      [direction]: (_b = options.distance) != null ? _b : 0
    }, options.offset)
  })));
  watch2(() => state.arrivedState[direction], async (v) => {
    var _a22, _b2;
    if (v) {
    ...
    get(target2, prop, rec) {
      ...
      if (!(prop in refs)) {
        if (/[+_-]/.test(prop)) {
          ...
          refs[prop] = computed2(() => keys2.every((key) => unref2(proxy[key])));
        } else {
          refs[prop] = ref2(false);
        }
      }
      const r = Reflect.get(target2, prop, rec);
      return useReactive ? unref2(r) : r;
    }
  });
  return proxy;
}
...
function useMemoize(resolver, options) {
  const initCache = () => {
    if (options == null ? void 0 : options.cache)
      return reactive2(options.cache);
    if (isVue22)
      return getMapVue2Compat();
    return reactive2(/* @__PURE__ */ new Map());
  };
...
function useObjectUrl(object) {
  ...
  watch2(() => unref2(object), (newObject) => {
    release();
    if (newObject)
...
function useOffsetPagination(options) {
  ...
  const pageCount = computed2(() => Math.max(1, Math.ceil(unref2(total) / unref2(currentPageSize))));
...
function usePointer(options = {}) {
  ...
  return __spreadProps$12(__spreadValues$52({}, toRefs(state)), {
    isInside
  });
}
var SwipeDirection;
...
function useSorted(...args) {
  ...
  if (!dirty)
    return computed2(() => sortFn([...unref2(source)], compareFn));
  watchEffect2(() => {
    const result = sortFn(unref2(source), compareFn);
    if (isRef2(source))
      source.value = result;
    else
...
function useSpeechSynthesis(text, options = {}) {
  ...
  const bindEventsForUtterance = (utterance2) => {
    utterance2.lang = unref2(lang);
    utterance2.voice = unref2(options.voice) || null;
...
function useTextareaAutosize(options) {
  ...
  watch2([input, textarea], triggerResize, { immediate: true });
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = computed2(() => getTimeAgo(new Date(resolveUnref(time)), unref2(now2.value)));
  if (exposeControls) {
...
function useTitle(newTitle = null, options = {}) {
  ...
  function format(t) {
    ...
    return isFunction(template) ? template(t) : unref2(template).replace(/%s/g, t);
  }
  watch2(title, (t, o) => {
    if (t !== o && document2)
...
function useTransition(source, options = {}) {
  ...
  const currentTransition = computed2(() => {
    const t = unref2(transition);
    return isFunction(t) ? t : createEasingFunction(t);
  });
  const sourceValue = computed2(() => {
    const s = unref2(source);
    return isNumber(s) ? s : s.map(unref2);
  });
  const sourceVector = computed2(() => isNumber(sourceValue.value) ? [sourceValue.value] : sourceValue.value);
  const outputVector = ref2(sourceVector.value.slice(0));
  let currentDuration;
  let diffVector;
  let endAt;
  let startAt;
  ...
  const start = () => {
    pause();
    currentDuration = unref2(duration);
  ...
  return computed2(() => {
    const targetVector = unref2(disabled) ? sourceVector : outputVector;
...
function useUserMedia(options = {}) {
  ...
  const videoDeviceId = ref2(options.videoDeviceId);
  const audioDeviceId = ref2(options.audioDeviceId);
  const { navigator = defaultNavigator } = options;
  const isSupported = useSupported(() => {
    var _a22;
  ...
  watch2([videoDeviceId, audioDeviceId], () => {
...
function useVModel(props, key, emit, options = {}) {
  ...
  if (passive) {
    ...
    watch2(() => props[key], (v) => proxy.value = cloneFn(v));
    watch2(proxy, (v) => {
      if (v !== props[key] || deep)
        _emit(event, v);
    }, { deep });
...
function useWatchForSizes(size, list, calculateRange) {
  watch2([size.width, size.height, list], () => {
    calculateRange();
  });
}
...
function useWindowScroll({ window: window2 = defaultWindow } = {}) {
  ...
  const x = ref2(window2.pageXOffset);
  const y = ref2(window2.pageYOffset);
...
//# sourceMappingURL=chunk-54TMIROV.js.map
